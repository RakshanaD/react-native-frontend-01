  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.courierList} contentContainerStyle={{ paddingBottom: 100 }}>
        {couriers.map((c) => (
          <TouchableOpacity key={c.name} style={styles.courierCard}>
            <Image source={c.logo} style={styles.logo} resizeMode="contain" />
            <View style={{ marginLeft: 12 }}>
              <Text style={styles.courierName}>{c.name}</Text>
              <Text style={styles.courierItems}>{c.items} Item{c.items !== 1 ? "s" : ""}</Text>
            </View>
          </TouchableOpacity>
        ))}
      ...
      <TouchableOpacity
        ...
        onPress={() => router.push("/confirm-courier" as any)}
      >
